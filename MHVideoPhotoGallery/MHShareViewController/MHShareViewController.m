    ...
    [self getAllImagesForSelectedRows:^(NSArray *images) {
        ...
        for (UIImage *img in images) {
            if ([img isKindOfClass:UIImage.class]) {
                ...
                [picker addAttachmentData:UIImageJPEGRepresentation(img, 1.0)
                                 mimeType:@"image/jpeg"
                                 fileName:@"image"];
            }
        }
...
-(void)saveImages:(NSArray*)object{
    [self getAllImagesForSelectedRows:^(NSArray *images) {
        for (UIImage *imageToStore in images) {
            
                ALAssetsLibrary* library = ALAssetsLibrary.new;
                NSData *data = UIImageJPEGRepresentation(imageToStore, 1.0);
